...
setwd("~/technoDev non-stationnary/deployment POMDP")
source("write_deployment_POMDPx.R")
...
ecosystem_dynamics <- function(x_t, r, K, phi_t, q, b, eta, dep_effect, time_step){
  if (phi_t >=1){
    phi_new <- max(phi_t-dep_effect, 0)
  } else {
    phi_new <- phi_t
  }
  return(x_t + time_step*(x_t*r*(1-x_t/K)-(phi_new/eta*x_t**q)/(x_t**q+b**q)))
}
...
GAMMA <- 0.99
file_name <- paste0("actions_",paste0(DEP_EFFECT, collapse = "_"),
