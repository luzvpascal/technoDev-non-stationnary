...
voi_data_uncertain_response <- read.csv(
  paste0("res/value_uncertain_response_deployment_stationary_strategy_gamma_",gamma,".csv"),
  header = TRUE)

res_file <- paste0("res/value_uncertain_response_deployment_stationary_strategy_solution_gamma_",gamma,".csv")
...
for (index_exp in seq_along(experiments)){
  ...
  filtered_voi_data_uncertain_response$scen_test <- "best stationary"
