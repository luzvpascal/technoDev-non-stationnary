...
voi_temperatures_r_EVPI <- voi_data_full %>%
  ggplot(aes(x = delta_t_crit_K, y = r_EVPI * 100)) +
  geom_line(size=2) +
  facet_wrap(~scen, ncol = 5)+
...
ggsave(plot = voi_temperatures_best_perf,
       filename = "figures_onlyK/voi_cost_of_stationarity_dep_best_strategies.pdf",
       width = 20,
       height = 20,
       units = "cm")
